<template>
  <section class="py-16 sm:py-20">
    <div class="v mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      <div class="flex flex-col items-center gap-12 lg:gap-20">
        <h2
          class="mx-auto max-w-xl text-center text-3xl font-medium tracking-tight sm:text-4xl"
        >
          {{ $t("homePage.testimonials") }}
        </h2>
        <div
          class="grid divide-y divide-primary-900/10 dark:divide-primary-300/10 lg:grid-cols-3 lg:divide-x lg:divide-y-0"
        >
          <figure
            v-for="testimonial in testimonials"
            class="mx-auto flex max-w-lg flex-col items-center gap-6 py-8 lg:px-8 lg:py-0"
          >
            <div class="flex flex-row gap-2">
              <svg
                v-for="n in 3"
                class="h-6 w-6 text-primary-600 dark:text-primary-400"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
                aria-hidden="true"
              >
                <path
                  d="M12.0006 18.26L4.94715 22.2082L6.52248 14.2799L0.587891 8.7918L8.61493 7.84006L12.0006 0.5L15.3862 7.84006L23.4132 8.7918L17.4787 14.2799L19.054 22.2082L12.0006 18.26Z"
                />
              </svg>
            </div>
            <blockquote
              class="text-center text-lg italic leading-loose text-primary-950/70 dark:text-primary-200/70"
            >
              <p>“{{ $t(`homePage.${testimonial.message}`) }}”</p>
            </blockquote>
            <figcaption class="flex flex-col gap-4">
              <div class="text-center">
                <div class="text-base font-medium">{{ testimonial.name }}</div>
                <div
                  class="text-sm text-primary-950/70 dark:text-primary-200/70"
                >
                  {{ testimonial.description }}
                </div>
              </div>
            </figcaption>
          </figure>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup>
const testimonials = ref([
  {
    name: "Rebecca Hayes",
    description: "Designer",
    message: "testimonial1",
  },
  {
    name: "Rosa Spencer",
    description: "Team Leader",
    message: "testimonial2",
  },
  {
    name: "Christina Upton",
    description: "Manager",
    message: "testimonial3",
  },
]);
</script>

<style scoped></style>
